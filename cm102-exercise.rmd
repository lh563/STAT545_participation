...
```{r}
#"" split the sentence chatacter by nothing
fox %>% 
  str_split(pattern="")
...
```{r}
fruit %>% head
#at least 7 characters, so we want to add some thing to reach 7
fruit %>% 
...
```{r}
#tell how many chracters in each vector(apple, banana......)
str_length(fruit)
# entry in the fruit vector
length(fruit)
```
...
test_that("folder names are length 5.", {
    expect_true(str_length(folders) == 6)
})
```
...
# i something a
# Italy disappear, because I is captalized
```
