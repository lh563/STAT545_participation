...
```{r}
as.character(100)#convert number to string
as.numeric("100")#convert string to number
...
```{r}
mtcars$hp#subset the columns from the dataframe of mtcars
```
...
```{r}
(course<-c("STAT","545"))
#"545" is a string now, cuz c() can only hold one type of data
```
...
```{r}
(a <- 7:-2) #print out the output by putting ()
(n <- length(a))
```
...
```{r}
#multiply first*1, second*2.....last*10
a*1:10
...
course # the object does not change
```
...
```{r}
my_list[2]
my_list[[2]]#just the contents/entry of first entry
my_list[[2]][1]#subset the first of the vector
```
...
```{r}
identical(list(4), 4)
#list has extra baggage around the object, while the vectors do
identical(c(4), 4)
```
