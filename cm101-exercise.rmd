...
# the return function will return the y value, or you can just type "y", it will also return
square <- function(x) {
  y <- x^2
  return(y)
}
...
mmm2 <- function(x) {
    if (!is.numeric(x)) {
        stop("Expecting x to be numeric. You gave me", typeof(x))
    }
    max(x) - min(x)
}
```
...
mm2((numeric(0)))
mmm2(gapminder)
mmm2(gapminder$country)#country=factors, and facotrs are integers
...
```{r}
#... means that you can put anything here, the ... in function should match ... in quantile. The order matters. The order should be same.
