...
# by putting integers, put the objects in order
```
...
#"sample"random sample,set.seed makes the randome result the same everytime
```
...
nlevels(draw)
fct_count(draw)
#first column: factor levels, second: level counts
```
...
#bindrows and rbind are different
#rbind assume that you want to add two levels of film together, but bindrows do not make assumptions, but it does not know you want to add two levels, so it makes the film into character factors and have a warning 
```
...
#all the countries are still there
...
gap_asia_2007$country %>% 
  ...
  levels() %>% 
  head()
#put Syria comes first over the other cuntries
#put Syria after second level
```
